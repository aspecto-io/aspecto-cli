...
const fetch = async (packageName: string, token: string): Promise<RouteTestEntry[]> => {
    ...
    const routes = (await client.get<Route[]>(`/routes?package=${packageName}`, { headers })).data
        .map((x: Route) => x._id)
        .filter((x: string) => x);
    ...
    const getRouteDetails = (routeName: string) =>
        client.get<RouteDetails[]>(`/route?route=${encodeURIComponent(routeName)}${query.join('')}`, { headers });
